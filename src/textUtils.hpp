...
/*
    TODO:
        - convert file content into class
        - store bitmaps in std::map, create bitmaps when first needed
        - destatic whatever possible
*/
...
static void applyColor(FT_Bitmap *bitmap, const float color_rgb)
{
    const int width = bitmap->width;
    const int height = bitmap->rows;
...
static void drawText(const float x, const float y, const char *text,
                     ...
                     [[maybe_unused]] const float boldness = NEUTRAL)
{
    if (text != nullptr)
    {
