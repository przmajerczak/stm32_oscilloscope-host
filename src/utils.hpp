...
static int scaleAdcValueToY(const VerticalBoundsData &verticalBoundsData, const uint16_t adc_value)
{
    ...
    float scaled_adc_value{
        (adc_value - verticalBoundsData.vertical_lower_bound) * factor};
...
static int scaleYToVoltage_mV(const VerticalBoundsData &verticalBoundsData, const int y)
{
    const float current_vertical_display_resolution{
...
static uint16_t scaleYToAdcWithinBounds(const VerticalBoundsData &verticalBoundsData, const int y)
{
    const float current_vertical_display_resolution{
        verticalBoundsData.vertical_upper_bound -
        verticalBoundsData.vertical_lower_bound};

    return verticalBoundsData.vertical_lower_bound + yAsPercentOfMaxY(y) * current_vertical_display_resolution;
}
